...
class Dice():
    ...
    def __str__(self):
        return "Last roll: " + str(self.roll_tracker[-1])
    ...
    def roll(self):
        num = random.randrange(1, self.sides + 1)
        self.roll_tracker.append(num)
        return num
    ...
    def print_count_for_num(self, passed_num):
        self.passed_num = passed_num
        count = 0
        for i in range(len(self.roll_tracker)):
            if self.roll_tracker[i] == self.passed_num:
                count += 1
        print(str(passed_num) + " was rolled " + str(count) + " times")
